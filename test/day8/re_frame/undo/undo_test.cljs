...
(defn undo-fixtures
  ...
  (let [restore-re-frame-fn (re-frame/make-restore-fn)]
    ...
    (undo/clear-history!)
    (try (f) (finally (restore-re-frame-fn)))))
...
(deftest test-undos
  ...
  ;; Check the undo state is
  ;; correct
  (is (undo/undos?))
  (is (not (undo/redos?)))
  (is (= [5 6 7 8 9 10] (undo/undo-explanations)))
  (is (= {db/app-db [{:test 5} {:test 6} {:test 7} {:test 8} {:test 9}]}
...
(deftest test-undos-interceptor
  (re-frame/reg-event-db :change-db
    (undo/undoable "change-db")
    (fn [db _] (update db :test inc)))
  (doseq [i (range 10)] (re-frame/dispatch-sync [:change-db]))
  ;; Check the undo state is correct
  (is (undo/undos?))
  (is (not (undo/redos?)))
  (is (= ["change-db" "change-db" "change-db" "change-db" "change-db"
          "change-db"]
